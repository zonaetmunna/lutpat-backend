...
const createCategory = async (req, res, next) => {
  try {
    ...
    await category.save();
    return res.status(201).json(responseGenerate(category, 'Category Added successfully!', false));
...
const deleteCategory = async (req, res, next) => {
  try {
    ...
    if (category.deletedCount) {
      return res.status(200).json(responseGenerate(null, 'Category Deleted successfully!', false));
    };

    return res.status(404).json(responseGenerate(null, 'No Category found with this Id!', true));
